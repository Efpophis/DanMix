...
def build_layout(files):
    master_controls = [
                    [sg.Slider((0,100), orientation='vertical', expand_y=True, default_value=75, enable_events=True, disable_number_display=True, key=f'Vol::master')],
                     #sg.Text('    '),
    ...
    while True:
        ...
        if event == sg.WIN_CLOSED or event == 'Exit':
            ...
            if 'Psr::' in event:
                file = event[5:]
                if file == 'master':
                    print('master pause not implemented yet')
